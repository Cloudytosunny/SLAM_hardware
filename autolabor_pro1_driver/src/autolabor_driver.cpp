...
void ChassisDriver::send_speed_callback(const ros::TimerEvent &)
{
  ...
  {
    linear_speed = current_twist_.linear.x/8;
    angular_speed = current_twist_.angular.z/8;
  }
  else
  {
    linear_speed = 0;
...
void ChassisDriver::handle_speed_msg(uint8_t *buffer_data)
{
  ...
  ROS_DEBUG_STREAM("receive -> left: " << delta_left_ << "(" << rev_left_ << ")"
                                       << "; right: " << delta_right_ << "(" << rev_right_ << ")");
...
void ChassisDriver::run()
{
  ...
  if (init())
  {
    ...
    ros::Subscriber cmd_sub = node.subscribe<geometry_msgs::Twist>("/turtle1/cmd_vel", 10, &ChassisDriver::twist_callback, this);
